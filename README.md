...
# Обучение с resnet18 
Я уже говорил, что планируется использовать мою архитектуру с resnet18, однако, выяснилось кое-что очень странное.
Новая архитектура представлена в файлах new_model_resnet18.py и обучение в файле new_train.py. 

# Предикты
Посмотрим на предикты модели после 5, 10, 15, 20 эпох.
Дальше я не дообучал модель т.к. не вижу смысла, моя архитектура написанная вручную справилась лучше, но стоит сказать, что очень велик шанс того, что я неправильно настроил модель.

Однако, перейдем к результатам:

  - Эпоха 5:
    <img width="2400" height="600" alt="debug_prediction_resnet_5" src="https://github.com/user-attachments/assets/9050a4c6-2f0a-4137-99a0-564f5020013f" />
    
  - Эпоха 10:
    <img width="2400" height="600" alt="debug_prediction_resnet_10" src="https://github.com/user-attachments/assets/089a0c89-7c52-4e5e-abaf-21ee416ba7d6" />

  - Эпоха 15:
    <img width="2400" height="600" alt="debug_prediction_resnet_15" src="https://github.com/user-attachments/assets/c8133c93-cdf3-464d-8d8a-109d7dca6b61" />

  - Эпоха 20:
    <img width="2400" height="600" alt="debug_prediction_resnet_20" src="https://github.com/user-attachments/assets/a1c4146f-5ee0-4c7a-b844-8db01fcc86f6" />

Но стоит сказать, что финальная версия модели, показала некий маленький результат:
<img width="2400" height="600" alt="debug_prediction_resnet_final" src="https://github.com/user-attachments/assets/4532bd7f-3327-4a09-bfa6-56b980d51e40" />

Веса выкладывать не имеет смыла т.к. модель бесполезна сама по себе. Она просто предсказала облако без четких границ трещины.

# Правка 
Стоит сказать, что после вторичного анализа исследования моей первой модели я все же выяснил, что после пяти эпох обучения модель показывает довольно хороший результат, правда надо делать бинаризацию, но это не имеет значения, она почти идельно выделила трещину. Ее веса я тоже выложил на Google Disc
