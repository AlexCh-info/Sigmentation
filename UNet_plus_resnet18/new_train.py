...
from new_model_resnet18 import UNet_ResNet
...
def plot_training_history(history, save_path='D:/portfolio/project1_Sigmentation/src/debug_png/res_net_training_history_progress.png'):
...
# Перед началом обучения (эпохи 0-5)
def freeze_encoder(model):
    for param in model.initial.parameters(): param.requires_grad = False
...
# После 5-й эпохи (разморозка для тонкой настройки)
...
def train(config_path="D:/portfolio/project1_Sigmentation/configs/config.yaml"):
    ...
    optimizer = torch.optim.Adam([
        {'params': model.initial.parameters(), 'lr': 1e-4},
        {'params': model.layer1.parameters(), 'lr': 1e-4},
        {'params': model.layer2.parameters(), 'lr': 1e-4},
        {'params': model.layer3.parameters(), 'lr': 1e-4},
        {'params': model.layer4.parameters(), 'lr': 1e-4},
        # Декодер обучаем быстрее
        {'params': model.up1.parameters(), 'lr': 1e-3},
        {'params': model.up2.parameters(), 'lr': 1e-3},
        {'params': model.up3.parameters(), 'lr': 1e-3},
        {'params': model.up4.parameters(), 'lr': 1e-3},
        {'params': model.up5.parameters(), 'lr': 1e-3},
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        try:
            if epoch % 5 == 0:
                save_checkpoint(
                    model, optimizer, epoch, val_metrics,
                    f"{config['paths']['weights_dir']}/res_net_checkpoint_epoch_{epoch}.pth"
                )
    ...
    try:
        ...
        save_checkpoint(
            ...
            f"{config['paths']['weights_dir']}/resnet_final_model.pth"
        )
